  ...
  const { mutate: addToCart } = useMutation({
    mutationFn: async (cartItem: any) => {
      try {
        ...
        // Debugging: Log lỗi để dễ dàng tìm nguyên nhân
        console.error("API Error:", error.response?.data || error.message);
        throw error; // Rethrow error to handle in onError
      }
    },
    onSuccess: () => {
  ...
  return (
    <div>
      ...
      {selectedVariant?.colors && selectedVariant.colors.length > 0 ? (
        ...
        <p className="text-gray-500">Không có màu sắc khả dụng cho kích thước này.</p>
      )}
      <Button
        type="primary"
